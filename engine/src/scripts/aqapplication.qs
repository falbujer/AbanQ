...
function aqAppScriptMain()
{
  // ###
  return;
  // Para desarrollo. El script se edita a si mismo.
  if (MessageBox.Yes ==
      MessageBox.information("Mostrar Código ?",
                             MessageBox.Yes, MessageBox.No,
                             MessageBox.NoButton)) {
    var aqAppScr = QSProject.script("aqAppScript");
    aqAppScr.setFileName(
      "src/scripts/aqapplication.qs"
    );
    var wb = QSProject.workbench();
    wb.open();
    wb.showScript(aqAppScr);
  }
  // ###
}
