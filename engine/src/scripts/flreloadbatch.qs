  ...
  if (resComando.ok == false) {
    MessageBox.warning(
      util.translate("scripts",
                     "Error al buscar los módulos en el directorio:\n") + dirMods,
...
function oficial_ejecutarComando(comando: String) : Array {
  ...
  if (Process.stderr != "")
  {
    ...
    if (this.iface.pub_log) {
      this.iface.pub_log.child("log").append(
...
function oficial_cargarModulo(nombreFichero: String) : Boolean {
  ...
  if (xmlModule.setContent(f))
  {
    ...
    if (nodeModule.namedItem("flversion"))
      versionMinimaFL = nodeModule.namedItem("flversion").toElement().text();
  ...
  if (this.iface.compararVersiones(versionSys, versionMinimaFL) == 2)
  {
    var contVersion = MessageBox.warning(
      util.translate("scripts", "Este módulo necesita la versión ") +
      versionMinimaFL +
      util.translate("scripts", " o superior de la aplicación base,\nactualmente la versión instalada es la ") +
      sys.version() +
...
function oficial_traducirCadena(cadena: String, path: String, modulo: String) : String {
  ...
  if (cadena.find("\"AbanQ") != -1)
    cadena = cadena.mid(27, cadena.length - 29);
  else
