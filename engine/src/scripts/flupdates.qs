...
function interna_main()
{
  ...
  if (MessageBox.Yes == MessageBox.warning(
        this.iface.util.translate(
          "scripts",
          "Se va a proceder a conectar a través de Internet\n" +
    ...
    MessageBox.information(
      this.iface.util.translate("scripts",
                                "Actualización cancelada."), MessageBox.Ok
    );
  }
}
  ...
  if (sys.addSysCode(this.iface.codeBuffer)) {
    ...
    MessageBox.critical(
      this.iface.util.translate(
        ...
        "puede obtener ayuda y soporte en http://www.infosial.com InfoSiAL S.L."
      ), MessageBox.Ok
    );
  }
}
...
function oficial_progresoDatos(bytesDone, bytesTotal)
{
  ...
  this.iface.util.setLabelText(
    this.iface.util.translate(
      "scripts", "Descargando %1 de %2"
    ).arg(bytesDone).arg(bytesTotal)
  );
}
