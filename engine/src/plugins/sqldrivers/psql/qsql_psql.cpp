                            ...
                            PGresult *result = 0)
{
  ...
  origMsg += "\n\n";
  origMsg += QApplication::tr("Obtenga ayuda y soporte en");
  origMsg += " http://www.infosial.com\n(c) InfoSiAL S.L.";
...
bool QPSQLResult::nextResult(int i)
{
  ...
  if (flres) {
    ...
    if (!isForwardOnly()) {
      ...
      if (status != PGRES_COMMAND_OK) {
        //setLastError(
        qMakeError(QApplication::tr("No se ha podido mover a una posici�n del cursor %1")
                   .arg(d->idCursor), QSqlError::Statement, d, q, result);
        //);
        PQclear(result);
        return false;
      ...
      qWarning(QApplication::tr("Esta consulta s�lo puede posicionarse hacia adelante (isForwardOnly)"));
      qWarning(d->qry);
      return false;
    }
...
void QPSQLResult::calcSize()
{
  if (isSelect() && !d->qry.isEmpty()) {
    if (!d->idCursor.isEmpty() && !isForwardOnly() && isCursorValid()) {
      ...
      if (status != PGRES_COMMAND_OK) {
        //setLastError(
        qMakeError(QApplication::tr("No se ha podido calcular el tama�o del cursor %1")
                             ...
                             const QString &host, int port)
{
  ...
  if (protocol() < QPSQLDriver::Version74) {
    msgBoxCritical(tr("Conexi�n fallida"),
                   tr("La versi�n de PostgreSQL es inferior a la 7.4.\nSe necesita PostgreSQL 7.4 o superior."));
    close();
    setOpenError(true);
    return false;
  }
  ...
  if (!open(db, user, password, host, port, db_->connectOptions())) {
    if (lastError().type() == QSqlError::Connection) {
      if (db_->interactiveGUI() && lastError().text().contains(db)) {
        ...
        if (res == QMessageBox::Yes) {
          ...
          if (lastError().type() != QSqlError::None) {
            msgBoxCritical(tr("Creaci�n fallida"), tr("No se pudo crear la base de datos %1.").arg(db));
            msgBoxCritical(tr("Error"), QString(lastError().driverText()) + "\n" +
                           QString(lastError().databaseText()));
            delete qry;
            return false;
            ...
            if (!open(db, user, password, host, port, db_->connectOptions())) {
              msgBoxCritical(tr("Conexi�n fallida"),
                             tr("La base de datos %1 ha sido creada, pero no se pudo realizar la conexi�n.").arg(db));
              msgBoxCritical(tr("Error"), QString(lastError().driverText()) + "\n" +
                             QString(lastError().databaseText()));
              delete qry;
              return false;
      ...
      if (lastError().type() != QSqlError::None) {
        msgBoxCritical(tr("Conexi�n fallida"), tr("No se pudo conectar con la base de datos %1.").arg(db));
        msgBoxCritical(tr("Error"), QString(lastError().driverText()) + "\n" +
                       QString(lastError().databaseText()));
        return false;
      }
...
bool QPSQLDriver::alterTable2(const QString &mtd1, const QString &mtd2, const QString &key, bool force)
{
  ...
  if (constraintExists(constraintName) &&
      !q.exec("ALTER TABLE " + oldMTD->name() + " DROP CONSTRAINT " + constraintName)) {
#ifdef FL_DEBUG
    qWarning("FLManager : " + QApplication::tr("En m�todo alterTable, no se ha podido borrar el �ndice %1_pkey de la tabla antigua.").arg(oldMTD->name()));
#endif
...
#ifdef FL_DEBUG
        qWarning("FLManager : " +
                 QApplication::tr("En m�todo alterTable, no se ha podido borrar el �ndice %1_%2_key de la tabla antigua.")
...
bool QPSQLDriver::beginTransaction()
{
  ...
  if (PQresultStatus(res) != PGRES_COMMAND_OK) {
    setLastError(
      qMakeError(QApplication::tr("No se pudo iniciar transacci�n"),
...
bool QPSQLDriver::commitTransaction()
{
  ...
  if (PQresultStatus(res) != PGRES_COMMAND_OK) {
    setLastError(
      qMakeError(QApplication::tr("No se pudo aceptar la transacci�n"),
...
bool QPSQLDriver::rollbackTransaction()
{
  ...
  if (PQresultStatus(res) != PGRES_COMMAND_OK) {
    setLastError(
      qMakeError(QApplication::tr("No se pudo deshacer la transacci�n"),
...
void QPSQLDriver::createIndex(const QString &fieldName, const QString &tableName, bool textOp, bool noUpper) const
{
  ...
  if (d->checkLock) {
    ...
    if (idxs->next()) {
#ifdef FL_DEBUG
      qWarning("No se puede crear �ndice, tabla bloqueada : " + tableName);
#endif
      delete idxs;
      return;
    } else
...
void QPSQLDriver::Mr_Proper()
{
  ...
  FLUtil::createProgressDialog(tr("Regenerando �ndices"), qry.size());
  ...
  while (qry.next()) {
    ...
    FLUtil::setLabelText(tr("Regenerando �ndice %1.%2").arg(tableIdx).arg(fieldIdx));
#ifdef FL_DEBUG
    qWarning(tr("Regenerando �ndice %1.%2").arg(tableIdx).arg(fieldIdx));
#endif
    FLUtil::setProgress(++steps);
  ...
  if (sqlQuery.exec("select relname from pg_class where ( relkind = 'r' ) "
    ...
    while (sqlQuery.next()) {
      ...
      FLUtil::setProgress(++steps);
      FLUtil::setLabelText(tr("Creando �ndices para %1").arg(item));
