...
void FLAccessControlMainWindow::processObject(QObject *obj)
{
  ...
  for (; it.current(); ++it) {
		QObjectList *l = mw->queryList("QAction", it.currentKey(), false);
    QObjectListIt ito(*l);
    QAction *a;
		QString perm = *(*it);
    while ((a = ::qt_cast<QAction *>(ito.current())) != 0) {
...
public:
  ...
  */
  ...
  void setFromObject(QObject *);
	...
	void disableFLFormWidget(QWidget *);
...
void FLAccessControlForm::processObject(QObject *obj)
{
  ...
  if (!fm || !acosPerms_)
    ...
    QWidget *w;
      ...
      ++ito;
// 			qWarning("Objeto " + QString(w->name()) + " de tipo " + QString(w->className()));
// 			QString cN = QString(w->className());
// 			if (cN == "QTabWidget" || cN == "QTabBar" || cN == "QWidget" || cN == "QWidgetStackPrivate::Invisible" || cN == "QPushButton") {
// 				if (w->parentWidget()) {
// 					qWarning("pater " + QString(w->parentWidget()->className()));
// 				}
// 				w->setDisabled(false);
// 				continue;
// 			}
...
// 				w->setDisabled(true);
  ...
  if (perm_ == "r-") {
		QObjectList *l = fm->queryList("FLTableDB");
    QObjectListIt ito(*l);
    QWidget *w;
		QWidget *tDB;
		while ((tDB = ::qt_cast<QWidget *>(ito.current())) != 0) {
			++ito;
			tDB->setDisabled(false);
			::qt_cast<FLTableDB *>(tDB)->setReadOnly(true);
			QObjectList *lT = tDB->queryList("QWidget");
			QObjectListIt itoT(*lT);
			while ((w = ::qt_cast<QWidget *>(itoT.current())) != 0) {
				++itoT;
				w->setDisabled(false);
			}
			delete lT;
...
void FLAccessControlForm::disableFLFormWidget(QWidget *w)
{
	if (!w) {
		return;
	}
	QString cN = QString(w->className());
	QString n = QString(w->name());
qWarning(cN + "-" + n);
	if (cN == "QPushButton" && n == "pushButtonCancel") {
		w->setDisabled(false);
		return;
	}
	QString parentClass = w->parentWidget() ? QString(w->parentWidget()->className()) : "";
qWarning("parentClass " + parentClass ); 
	if (cN == "QTabWidget" || cN == "QTabBar" || cN == "QWidget") {
		w->setDisabled(false);
		return;
	}
	w->setDisabled(true);
